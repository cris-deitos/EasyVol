...
if (!$app->checkPermission('conventions', $requiredPermission)) {
    die('Accesso negato');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!$csrf->validateToken($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($errors)) {
            ...
            if ($result) {
                $redirectId = $isEdit ? $item['id'] : $result;
                header('Location: convention_view.php?id=' . $redirectId . '&saved=1');
                exit;
            } else {
...
$pageTitle = $isEdit ? 'Modifica Convenzione' : 'Nuova Convenzione';
