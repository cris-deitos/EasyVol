...
if ($surveillanceId > 0) {
    $sql = "SELECT * FROM junior_member_health_surveillance WHERE id = :id AND junior_member_id = :member_id";
    $existing = $db->fetch($sql, ['id' => $surveillanceId, 'member_id' => $memberId]);
    
    if ($existing) {
        $surveillance = $existing;
    } else {
        header('Location: junior_member_view.php?id=' . $memberId);
        exit;
    }
}
        ...
        if (empty($errors)) {
            try {
                ...
                if ($surveillanceId > 0) {
                    ...
                    $db->update('junior_member_health_surveillance', $data, ['id' => $surveillanceId]);
                } else {
                    // Insert new record
                    $data['junior_member_id'] = $memberId;
                    $data['created_by'] = $app->getUserId();
                    $db->insert('junior_member_health_surveillance', $data);
                }
                
                header('Location: junior_member_view.php?id=' . $memberId . '&tab=health-surveillance&success=1');
                exit;
            } catch (\Exception $e) {
