...
CREATE TABLE IF NOT EXISTS `members` (
  ...
  `notes` text,
  `privacy_consent_date` date DEFAULT NULL COMMENT 'Data consenso privacy',
  `privacy_consent_version` varchar(50) DEFAULT NULL COMMENT 'Versione informativa privacy',
  `data_processing_consent` tinyint(1) DEFAULT 0 COMMENT 'Consenso trattamento dati',
  `sensitive_data_consent` tinyint(1) DEFAULT 0 COMMENT 'Consenso dati sensibili',
  `marketing_consent` tinyint(1) DEFAULT 0 COMMENT 'Consenso marketing',
  `image_rights_consent` tinyint(1) DEFAULT 0 COMMENT 'Consenso diritti immagine',
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `created_by` int(11),
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `updated_by` int(11),
...
CREATE TABLE IF NOT EXISTS `junior_members` (
  ...
  `photo_path` varchar(255),
  `notes` text,
  `privacy_consent_date` date DEFAULT NULL COMMENT 'Data consenso privacy tutore',
  `privacy_consent_version` varchar(50) DEFAULT NULL COMMENT 'Versione informativa privacy',
  `data_processing_consent` tinyint(1) DEFAULT 0 COMMENT 'Consenso trattamento dati tutore',
  `sensitive_data_consent` tinyint(1) DEFAULT 0 COMMENT 'Consenso dati sensibili tutore',
  `marketing_consent` tinyint(1) DEFAULT 0 COMMENT 'Consenso marketing tutore',
  `image_rights_consent` tinyint(1) DEFAULT 0 COMMENT 'Consenso diritti immagine tutore',
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `created_by` int(11),
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `updated_by` int(11),
...
-- Simplified fee payment reminder system (uses existing email_queue infrastructure)
...
-- Insert default member qualification types
INSERT INTO `member_qualification_types` (`name`, `description`, `sort_order`, `is_active`) VALUES
('Volontario Operativo', 'Volontario operativo base', 10, 1),
('OPERATORE GENERICO', 'Operatore generico', 15, 1),
('PRESIDENTE', 'Presidente associazione', 20, 1),
('VICE PRESIDENTE', 'Vice presidente associazione', 30, 1),
('CAPOSQUADRA', 'Capo squadra', 40, 1),
('Coordinatore', 'Coordinatore di settore o territorio', 50, 1),
('RESPONSABILE RAPPORTI ISTITUZIONALI E STAMPA', 'Responsabile rapporti istituzionali e stampa', 60, 1),
('RESPONSABILE NUCLEO TLC RADIO', 'Responsabile nucleo TLC radio', 70, 1),
('RESPONSABILE NUCLEO GIS/GPS', 'Responsabile nucleo GIS/GPS', 80, 1),
('RESPONSABILE NUCLEO SEGRETERIA OPERATIVA', 'Responsabile nucleo segreteria operativa', 90, 1),
('RESPONSABILE NUCLEO DRONE', 'Responsabile nucleo drone', 100, 1),
('RESPONSABILE NUCLEO RICERCA E SOCCORSO', 'Responsabile nucleo ricerca e soccorso', 110, 1),
('RESPONSABILE NUCLEO NAUTICO', 'Responsabile nucleo nautico', 120, 1),
('RESPONSABILE NUCLEO SOMMOZZATORI', 'Responsabile nucleo sommozzatori', 130, 1),
('RESPONSABILE NUCLEO IDROGEOLOGICO', 'Responsabile nucleo idrogeologico', 140, 1),
('RESPONSABILE NUCLEO LOGISTICO', 'Responsabile nucleo logistico', 150, 1),
('RESPONSABILE NUCLEO CINOFILI', 'Responsabile nucleo cinofili', 160, 1),
('RESPONSABILE NUCLEO A CAVALLO', 'Responsabile nucleo a cavallo', 170, 1),
('RESPONSABILE NUCLEO CUCINA E MENSA', 'Responsabile nucleo cucina e mensa', 180, 1),
('Responsabile Formazione', 'Responsabile attività formative', 190, 1),
('Istruttore', 'Istruttore per corsi di formazione', 200, 1),
('OPERATORE SEGRETERIA', 'Operatore segreteria', 210, 1),
('OPERATORE TLC RADIO', 'Operatore TLC radio', 220, 1),
('OPERATORE GIS/GPS', 'Operatore GIS/GPS', 230, 1),
('OPERATORE DRONE', 'Operatore drone', 240, 1),
('OPERATORE CUCINA', 'Operatore cucina', 250, 1),
('OPERATORE LOGISTICO', 'Operatore logistico', 260, 1),
('OPERATORE IDROGEOLOGICO', 'Operatore idrogeologico', 270, 1),
('OPERATORE MENSA', 'Operatore mensa', 280, 1),
('OPERATORE SOMMOZZATORE', 'Operatore sommozzatore', 290, 1),
('OPERATORE NAUTICO', 'Operatore nautico', 300, 1),
('OPERATORE CINOFILO', 'Operatore cinofilo', 310, 1),
('OPERATORE A CAVALLO', 'Operatore a cavallo', 320, 1),
('OPERATORE FOTO REPORTER', 'Operatore foto reporter', 330, 1),
('AUTISTA A', 'Autista categoria A', 340, 1),
('AUTISTA B', 'Autista categoria B', 350, 1),
('AUTISTA C', 'Autista categoria C', 360, 1),
('AUTISTA D', 'Autista categoria D', 370, 1),
('AUTISTA E', 'Autista categoria E', 380, 1),
('PILOTA NATANTE', 'Pilota natante', 390, 1),
('Primo Soccorso', 'Operatore primo soccorso', 400, 1),
('NON OPERATIVO', 'Non operativo', 999, 1);

-- Insert default training course types
...
-- =============================================
-- ADVANCED DASHBOARD FEATURES (Migration 013)
-- =============================================
...
-- =============================================
-- GDPR COMPLIANCE FEATURES (Migration 014)
-- =============================================
...
-- =============================================
-- PASSWORD MANAGEMENT (Migration 017)
-- =============================================
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Gestione password condivise';

-- Password permissions table for individual password access control
CREATE TABLE IF NOT EXISTS `password_permissions` (
  ...
  FOREIGN KEY (`user_id`) REFERENCES `users`(`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Permessi individuali per password';
