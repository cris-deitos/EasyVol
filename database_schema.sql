...
-- =============================================
-- PRINT TEMPLATES SEED DATA
-- =============================================
-- These INSERT statements use the simplified schema format.
-- Templates are simple HTML-based with only 'single' or 'list' types.
...
CREATE TABLE IF NOT EXISTS `telegram_notification_config` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `action_type` varchar(100) NOT NULL COMMENT 'Type of action: member_application, junior_application, fee_payment, vehicle_departure, vehicle_return, event_created, scheduler_expiry, vehicle_expiry, license_expiry, qualification_expiry, course_expiry, user_login, health_surveillance_expiry, province_access',
...
-- Add newsletter management permissions (if they don't exist)
INSERT INTO `permissions` (`module`, `action`, `description`)
SELECT 'newsletters', 'view', 'Visualizzare le newsletter'
WHERE NOT EXISTS (SELECT 1 FROM `permissions` WHERE `module` = 'newsletters' AND `action` = 'view');

INSERT INTO `permissions` (`module`, `action`, `description`)
SELECT 'newsletters', 'create', 'Creare nuove newsletter'
WHERE NOT EXISTS (SELECT 1 FROM `permissions` WHERE `module` = 'newsletters' AND `action` = 'create');

INSERT INTO `permissions` (`module`, `action`, `description`)
SELECT 'newsletters', 'edit', 'Modificare newsletter bozza'
WHERE NOT EXISTS (SELECT 1 FROM `permissions` WHERE `module` = 'newsletters' AND `action` = 'edit');

INSERT INTO `permissions` (`module`, `action`, `description`)
SELECT 'newsletters', 'delete', 'Eliminare newsletter bozza'
WHERE NOT EXISTS (SELECT 1 FROM `permissions` WHERE `module` = 'newsletters' AND `action` = 'delete');
