-- Migration: Add password reset functionality and must_change_password field
-- Date: 2025-12-07
-- Description: Adds support for password reset tokens and forcing password change on first login

-- Add must_change_password field to users table
ALTER TABLE `users` 
ADD COLUMN `must_change_password` TINYINT(1) DEFAULT 0 COMMENT 'Flag to force password change on next login' AFTER `is_active`;
...
-- Update email_templates table structure if needed (check if 'name' column exists)
-- The EmailSender class references 'name' but schema has 'template_name'
-- We'll update the EmailSender to use template_name instead

-- Insert email templates for user management
